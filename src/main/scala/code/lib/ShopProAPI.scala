...
case class Shop() extends ShopProAPI
case class SalesStat() extends ShopProAPI
case class Sales() extends ShopProAPI
case class Customers() extends ShopProAPI

case class Products() extends ShopProAPI
case class ProductCollection(count: Int, data: List[Products])
object Products extends ShopProAPI {
  def getAll(params: Map[String,_] = Map.empty): ProductCollection = {
    request("GET", classURL, params).extract[ProductCollection]
  }
  /*
  def getOne(id: String): Products = {
    request("GET", instanceURL(id)).extract[Products]
  }
  def putOne(params: Map[String,_]): Products = {
    request("PUT", classURL, params).extract[Products]
  }
  */
}

case class Stocks() extends ShopProAPI
case class Categories() extends ShopProAPI
case class Payments() extends ShopProAPI
case class Deliveries() extends ShopProAPI
case class Gifts() extends ShopProAPI
case class Logout() extends ShopProAPI